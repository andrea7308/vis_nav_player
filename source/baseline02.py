...
class KeyboardPlayerPyGame(Player):
    ...
    def _build_graph(self):
        ...
        SIMILARITY_THRESHOLD = 0.10  # tune this threshold for more/less aggressive shortcut creation 1.0 = identical
        ...
        for rank, fi in enumerate(top_idx):
            ...
            if s < SIMILARITY_THRESHOLD:
                print(f"  Skipping #{rank+1}: similarity {s:.3f} is too low")
                continue
    ...
    def _get_current_node(self) -> int:
        """Find best-matching database nodes for current FPV."""
        feat = self.extractor.extract(self.fpv)
        sims = self.database @ feat
        
        # -------------------------------------------------------------------
        # Temporal Non-Maximum Suppression (NMS) for Top 3 Matches
        # -------------------------------------------------------------------
        sims_working = sims.copy()
        top_3_idx = []
        
        for _ in range(3):
            # 1. Find the current absolute best match
            best_idx = int(np.argmax(sims_working))
            top_3_idx.append(best_idx)
            
            # 2. Mask out nodes within 5 frames of this match
            lo = max(0, best_idx - 5)
            hi = min(len(sims_working), best_idx + 6) # +6 because slice end is exclusive
            sims_working[lo:hi] = -np.inf 
        
        best_match = top_3_idx[0]
        second_best_match = top_3_idx[1]
        
        # -------------------------------------------------------------------
        # Condition 1: Neighborhood Validation Check
        # -------------------------------------------------------------------
        NEIGHBORHOOD_THRESHOLD = 0.07  # tune this threshold based on validation set (lower = stricter) 
        n_nodes = len(sims)
        #neighbor_offsets = [-2, -1, 1, 2]
        neighbor_offsets = [-1, 1] # Only immediate neighbors for validation
        
        neighbors_pass = True
        for offset in neighbor_offsets:
            neighbor_idx = second_best_match + offset
            
            # Ensure the neighbor index is within database bounds
            if 0 <= neighbor_idx < n_nodes:
                if sims[neighbor_idx] < NEIGHBORHOOD_THRESHOLD:
                    neighbors_pass = False
                    break
            else:
                # Fail if we don't have enough neighbors to verify
                neighbors_pass = False
                break

        # -------------------------------------------------------------------
        # Condition 2: Proximity Override (Only if Condition 1 passed)
        # -------------------------------------------------------------------
        if neighbors_pass and hasattr(self, 'G') and self.G is not None and self.goal_node is not None:
            try:
                # Calculate the total weighted distance to the goal
                dist_best = nx.shortest_path_length(self.G, best_match, self.goal_node, weight="weight")
                dist_second = nx.shortest_path_length(self.G, second_best_match, self.goal_node, weight="weight")
                
                # Check if the valid 2nd best match is also closer to the destination
                if dist_second < dist_best:
                    print(f"[LOCALIZATION] Override: Switching to 2nd best match (Node {second_best_match})! "
                          f"Neighbors passed threshold AND it is closer to the goal ({dist_second:.1f} vs {dist_best:.1f}).")
                    return int(second_best_match)
                    
            except nx.NetworkXNoPath:
                # Fallback safely if one of the nodes is disconnected
                pass
                
        # Print standard tracking info if no override occurred
        if hasattr(self, 'G') and self.G is not None:
            print(f"[LOCALIZATION] Top matches: Node {top_3_idx[0]} ({sims[top_3_idx[0]]:.3f}), "
                  f"Node {top_3_idx[1]} ({sims[top_3_idx[1]]:.3f}), Node {top_3_idx[2]} ({sims[top_3_idx[2]]:.3f})")
    ...
    def display_next_best_view(self):
        ...
        if edge_info:
            ...
            if etype == "seq":
                #print(f"[PATH] Following predetermined sequence. Executing: {act}")
                print()
            elif etype == "vis":
                jump_target = path[1] # The node we are jumping to
                print(f"[ALERT] JUMPING to new location! Taking visual shortcut to Node {jump_target}.")
        else:
            hint = "AT GOAL"
        near = hops <= 5
