...
VISUAL_WEIGHT_BASE = 1.0    # base weight for visual shortcut edges
VISUAL_WEIGHT_SCALE = 40.0   # weight += scale * vlad_distance
MIN_SHORTCUT_GAP = 50       # minimum trajectory index gap for shortcuts

os.makedirs(CACHE_DIR, exist_ok=True)

# Helper function to calculate horizontal shift between two images (for visual jump direction)
import cv2
import numpy as np

def calculate_horizontal_shift(img1: np.ndarray, img2: np.ndarray) -> float:
    """
    Calculates the median horizontal shift between two images using SIFT feature matching.
    
    Args:
        img1: Current FPV image.
        img2: Target image (next node).
        
    Returns:
        float: Median shift in pixels. 
               Positive -> feature moved right (turn right).
               Negative -> feature moved left (turn left).
               0.0 -> Not enough matches to determine shift.
    """
    if img1 is None or img2 is None:
        return 0.0

    # Initialize SIFT detector (you can reuse the one from VLADExtractor if you make this a class method)
    ...
    # Need minimum descriptors to match
    ...
    # Require a minimum number of good matches to be confident
    if len(good_matches) < 8:
        return 0.0

    # Calculate the horizontal (X-axis) shift for each good match
    x_shifts = []
    for match in good_matches:
        x1 = kp1[match.queryIdx].pt[0] # X coord in current FPV
        x2 = kp2[match.trainIdx].pt[0] # X coord in target image
        x_shifts.append(x2 - x1)

    # Return the median shift to ignore extreme outliers
    return float(np.median(x_shifts))
...
class KeyboardPlayerPyGame(Player):
    ...
    def check_for_180_turn(self, cur_node: int, lookahead: int = 100, match_threshold: float = 0.30) -> bool:
        """
        Extracts the left/right 1/5th of the FPV, mirrors them, and checks the 
        next `lookahead` nodes. A high percentage match indicates a dead-end reversal.
        """
        if self.fpv is None or cur_node >= len(self.file_list) - 1:
            return False

        h, w = self.fpv.shape[:2]
        slice_w = w // 5

        # 1. Extract the left and right 1/5th of the current view
        left_crop = self.fpv[:, :slice_w]
        right_crop = self.fpv[:, w - slice_w:]

        # 2. Mirror them horizontally
        left_mirrored = cv2.flip(left_crop, 1)
        right_mirrored = cv2.flip(right_crop, 1)

        sift = cv2.SIFT_create()
        kp_lm, des_lm = sift.detectAndCompute(left_mirrored, None)
        kp_rm, des_rm = sift.detectAndCompute(right_mirrored, None)

        # Ensure we have enough features to make a valid percentage comparison
        if des_lm is None or des_rm is None or len(des_lm) < 5 or len(des_rm) < 5:
            return False

        bf = cv2.BFMatcher()
        end_node = min(cur_node + lookahead + 1, len(self.file_list))

        # 3. Search the next 100 nodes (stride of 3 to save frame rate)
        for i in range(cur_node + 1, end_node, 3):
            future_img = self._load_img(i)
            if future_img is None:
                continue

            future_left = future_img[:, :slice_w]
            future_right = future_img[:, w - slice_w:]

            _, des_fl = sift.detectAndCompute(future_left, None)
            _, des_fr = sift.detectAndCompute(future_right, None)

            if des_fl is None or des_fr is None or len(des_fl) < 2 or len(des_fr) < 2:
                continue

            # Old Left (mirrored) becomes New Right
            matches_l_to_r = bf.knnMatch(des_lm, des_fr, k=2)
            good_l_to_r = [m for m_n in matches_l_to_r if len(m_n) == 2 for m, n in [m_n] if m.distance < 0.75 * n.distance]

            # Old Right (mirrored) becomes New Left
            matches_r_to_l = bf.knnMatch(des_rm, des_fl, k=2)
            good_r_to_l = [m for m_n in matches_r_to_l if len(m_n) == 2 for m, n in [m_n] if m.distance < 0.75 * n.distance]

            # Calculate match percentage relative to the current FPV features
            pct_l = len(good_l_to_r) / len(des_lm)
            pct_r = len(good_r_to_l) / len(des_rm)

            if pct_l > match_threshold and pct_r > match_threshold:
                return True
    ...
    def display_next_best_view(self):
        ...
        for p in range(N_PREVIEW):
            if p < len(preview):
                ...
                if etype == "seq":
                    ...
                    # Load the un-resized target image for matching
                    target_full_img = self._load_img(preview[p])
                    
                    # Calculate horizontal shift
                    shift = calculate_horizontal_shift(self.fpv, target_full_img)
                    
                    # Determine action based on pixel shift threshold
                    # You may need to tune the '50' pixel threshold based on your resolution
                    if shift > 50:
                        lbl = "-> TURN RIGHT"
                    elif shift < -50:
                        lbl = "<- TURN LEFT"
                    elif shift == 0.0:
                        lbl = "? VISUAL JUMP" # Fallback if matching fails
                    else:
                        lbl = "^ FWD / V BACK" # Shift is small, likely aligned
        ...
        # NEW: 180 Dead-End Check appended to the output
        if self.check_for_180_turn(cur):
            print("\n" + "="*50)
            print("🚨 HIGH PERCENTAGE MIRROR MATCH DETECTED 🚨")
            print("         --> DO A 180 <--")
            print("="*50 + "\n")
