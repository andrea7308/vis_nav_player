...
class KeyboardPlayerPyGame(Player):
    ...
    def __init__(self, n_clusters: int = 128, subsample_rate: int = 5,
        ...
        self.cnn = ResNetEmbedding().to(self.device)
        self.cnn.load_state_dict(torch.load("maze_resnet_embedder.pth", 
    ...
    def act(self):
        ...
        # Localize
        feat = self._embed_img(self.fpv)
        cur = int(np.argmax(self.database @ feat))

        # Check if close enough to goal
        goal_sim = float(self.database[self.goal_node] @ feat)
        if goal_sim > 0.92:          # tune this threshold
            return Action.CHECKIN

        # Plan
        path = self._get_path(cur)
        if len(path) < 2:
            return Action.CHECKIN    # already at goal node

        next_node = path[1]
        edge = self.G[cur][next_node]

        if edge.get("edge_type") == "temporal":
            action_str = self._edge_action(cur, next_node)
            return ACTION_MAP.get(action_str, Action.IDLE)
        else:
            # Visual shortcut — the graph says "jump here" but the agent must
            # physically walk. Find the action toward next_node via the sub-path.
            try:
                sub = nx.shortest_path(self.G, cur, next_node,
                                    weight=None)   # hop-count only
                if len(sub) >= 2:
                    action_str = self._edge_action(sub[0], sub[1])
                    return ACTION_MAP.get(action_str, Action.IDLE)
    ...
    def _build_graph(self):
       ...
       # Load geometry-verified shortcuts from the mining step
        shortcuts_path = "visual_shortcuts.json"
        if os.path.exists(shortcuts_path):
            with open(shortcuts_path) as f:
                shortcuts = json.load(f)
            added = 0
            for i_str, targets in shortcuts.items():
                i = int(i_str)
                if i >= n:
                    continue
                for j in targets:
                    if j < n and abs(i - j) > MIN_SHORTCUT_GAP:
                        # These are already RANSAC-verified, so use a low weight
                        self.G.add_edge(i, j, weight=1.5, edge_type="visual")
                        added += 1
            print(f"Loaded {added} shortcut edges from {shortcuts_path}")
    ...
    def _setup_goal(self):
        ...
        if not targets:
            return
        feat = self._embed_img(targets[0])   # front-view target
        sims = self.database @ feat
        self.goal_node = int(np.argmax(sims))
        print(f"Goal node: {self.goal_node}, sim={sims[self.goal_node]:.3f}")
    ...
    def _embed_img(self, img_bgr: np.ndarray) -> np.ndarray:
        """BGR numpy → 128-d unit vector via CNN."""
        rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
        pil = Image.fromarray(rgb)
        t = self.transform(pil).unsqueeze(0).to(self.device)
        with torch.no_grad():
            v = self.cnn(t).squeeze(0).cpu().numpy()
        return v

    def _build_database(self):
        cache_file = os.path.join(CACHE_DIR, "cnn_database.npy")
        if os.path.exists(cache_file):
            self.database = np.load(cache_file)
            print(f"Loaded CNN database: {self.database.shape}")
            return
        vecs = []
        for path in tqdm(self.file_list, desc="CNN embed"):
            img = cv2.imread(path)
            vecs.append(self._embed_img(img) if img is not None else np.zeros(128))
        self.database = np.array(vecs)
        np.save(cache_file, self.database)
